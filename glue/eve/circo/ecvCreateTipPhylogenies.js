...
var outputPath = "trees/eve-lineage/";
var alignOutputPath = "alignments/export/";

// Get list of alignments
alignmentResult = glue.tableToObjects(glue.command(["list", "alignment", "-w", "name like '%AL_ecv-circo%'"]));
...
_.each(alignmentResult, function(alnObj) {
	...
	if (numTaxa >= 4) {
		...
		glue.command(["compute", "alignment", alignmentName, "cressMafftAligner"]);
  
		// Record feature coverage in each alignment
		glue.inMode("/module/cressFeaturePresenceRecorder", function() {		
		...
		glue.inMode("/module/ecvFigTreeAnnotationExporterDigs", function() {
